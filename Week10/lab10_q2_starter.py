...
def record_attempt(username, success):
    """Records a single login attempt with a timestamp."""
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
...
def get_failed_attempts(username):
    """Retrieves all rows where a specific user failed to log in."""
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
...
def count_failures_per_user():
    """Returns a list of tuples: (username, failure_count)."""
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
...
def delete_old_attempts(username):
    """Removes all records for a user and returns the number of rows removed."""
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
